...
data = data |>
  ...
  mutate(log.con.percapita = log(((data$total.con+1) / data$pop)*10000)) %>% 
  mutate(total.con.pertenthousand = data$total.con.percapita*10000)
...
data2 = data %>% 
  select("year", "spei", "pop", "dep.index", "built", "temp.jan", "temp.apr", "temp.jul", "temp.oct", "precip.jan", "precip.apr", "precip.jul", "precip.oct", "log.con.percapita", "total.con")
